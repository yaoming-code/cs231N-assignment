...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        y2, cache2 = affine_forward(y1, w2, b2)
        scores = y2
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range (self.num_layers - 1):
          ...
          out, cache = affine_relu_forward(outs[i], W, b)
          outs.append(out)
          caches.append(cache)
        scores, cache = affine_forward(outs[-1], self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        caches.append(cache)
        if y is None:
          return scores
        ############################################################################
        ...
        for i in range(self.num_layers - 1):
          dx, dw, db = affine_relu_backward(dx, caches[self.num_layers - i - 2])
          dw += self.reg * self.params[f'W{self.num_layers - i - 1}']
